...
export const useListCategory = () => {
    const {
        ...
        params,
        count
    } = useStoreApp(state => state.ListCategory)
    ...
    const onRefresh = async () => {
        ...
        if (count_time > time_click || time_request == 0) {
            ...
            await dispatch(stores.getListCategory({ page: 1, limit, ...paramsRef.current }))
            await dispatch(stores.getCountListCategory({ ...paramsRef.current }))
    ...
    const onLoadMore = async () => {
        ...
        await dispatch(stores.getListCategory({ page: page + 1, limit, ...paramsRef.current }))
        await dispatch(stores.updateSate({ loading_more: false }))
    }
    useEffect(() => {
        let count_time = new Date().getTime() - time_request
        if (count_time > time_expired || time_request == 0)
            onRefresh()
    }, [time_request])
    return {
        data,
