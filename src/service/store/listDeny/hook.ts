...
export const useListDeny = () => {
    const {
        ...
        params,
        count
    } = useStoreApp(state => state.ListDeny)
    ...
    const onRefresh = async () => {
        ...
        if (count_time > time_click || time_request == 0) {
            ...
            await dispatch(stores.getListDeny({ page: 1, limit, ...paramsRef.current }))
            await dispatch(stores.getCountListDeny({ ...paramsRef.current }))
    ...
    const onLoadMore = async () => {
        ...
        await dispatch(stores.getListDeny({ page: page + 1, limit, ...paramsRef.current }))
