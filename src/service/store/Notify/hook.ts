...
export const useNotifyApp = () => {
    const {
        ...
        params,
        count
    } = useStoreApp(state => state.Notify)
    ...
    const onRefresh = async () => {
        ...
        if (count_time > time_click || time_request == 0) {
            ...
            await dispatch(stores.getListNotify({ page: 1, limit, ...paramsRef.current }))
            await dispatch(stores.getCountNotify({ ...paramsRef.current }))
    ...
    const onLoadMore = async () => {
        ...
        await dispatch(stores.getListNotify({ page: page + 1, limit, ...paramsRef.current }))
