...
export const useListBooking = () => {
    const {
        ...
        params,
        count
    } = useStoreApp(state => state.ListBooking)
    ...
    const onRefresh = async () => {
        ...
        if (count_time > time_click || time_request == 0) {
            ...
            await dispatch(stores.getListBooking({ page: 1, limit, ...paramsRef.current }))
            await dispatch(stores.getCountListBooking({ ...paramsRef.current }))
    ...
    const onLoadMore = async () => {
        ...
        await dispatch(stores.getListBooking({ page: page + 1, limit, ...paramsRef.current }))
