...
export const useListAllocation = () => {
    const {
        data,
        loading,
        loading_more,
        ...
        params,
        count
    } = useStoreApp(state => state.ListAllocation)
    ...
    const onRefresh = async () => {
        ...
        if (count_time > time_click || time_request == 0) {
            await dispatch(stores.updateState({ loading: true }))
            await dispatch(stores.getListAllocation({ page: 1, limit, ...paramsRef.current }))
            await dispatch(stores.getCountListAllocation({ ...paramsRef.current }))
    ...
    const onLoadMore = async () => {
        ...
        await dispatch(stores.getListAllocation({ page: page + 1, limit, ...paramsRef.current }))
