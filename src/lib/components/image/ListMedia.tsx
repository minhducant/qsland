...
import { IconC, Touch } from './dependencies';
  ...
  return (
    <ScrollView style={[styles.scrollView, styleScrollView]} pagingEnabled horizontal showsHorizontalScrollIndicator={false} {...propsScrollView}>
      {listMedia.map((item, index) => (
        <Touch
          key={index}
          onPress={() => onPressItem && onPressItem({ item, index, seek: refVideoSeek.current[index]?.getSeek() })}
        ...
        >
          ...
          {item.type == 2 && Platform.OS == "ios" &&
            <VideoIOS
              isLive={isLive}
              ref={ref => refVideoSeek.current[index] = ref}
              autoPause={autoPause}
              index={index}
              source={{ uri: item.uri }}
              width={100}
