...
export default function ItemLockApartment({ item, onPress }: DefineItem<MappedItem<typeof _key>>) {
    ...
    return (
        <Touch onPress={onPress} styleBox={styles.container} pad10 white padT20 row>
            ...
            <Block flex1 marL10>
                <TextApp toUpperCase bold >{item?.code}</TextApp>
                <TextApp >{category_name}</TextApp>
            </Block>
            <Block alignItems='flex-end'>
                <TextApp bold primary>{trang_thai?.name}</TextApp>
                <TextApp >{AppDate.format2(item?.created_at)}</TextApp>
            </Block>
        </Touch>
    )
}
