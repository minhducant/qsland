...
const initOptions = {
  category_id: { name: '', value: null },
}
const ModalListProject = React.forwardRef<any, any>(
  ({ onSelected, onRemove }, ref) => {
    ...
    const categoryData = useListCategories()
    const categoryRef = useRef<any>(null)

    const onSave = () => {
      onSelected({
        category_id: options.category_id.value,
      })
    ...
    function onCategory() {
      ...
      if (category.size > 0) {
        let _name =
          category.size > 1
            ? `${category.size} Dự án đã chọn`
            : category.first()?.name
        setOptions(prev => ({
          ...prev,
          category_id: { value: JSON.stringify(_category), name: _name },
        }))
      } else {
        setOptions(prev => ({
          ...prev,
          category_id: { value: null, name: null },
        }))
      }
    ...
    return (
      <BottomSheet
        ...
        <ScrollView>
          <Block pad10>
            <ItemOption
              value={options.category_id.name}
              title={AppLang('du_an')}
        ...
        <ActionSheetIos
          onRefresh={categoryData.onRefresh}
          onPressButton={Press}
