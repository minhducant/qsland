...
export const uriImg = (uri: any, defaultImg = { uri: noimage }) => {
  if (isEmpty(uri) || !isString(uri)) return defaultImg
  if (uri.search('http') == -1) return defaultImg
  return { uri: convertHttpToHttps(uri) }
...
export const dateFormat = (date: any, type = true,) => {
  if (!date) return ''
  if (type)
...
export const dateFM = (date: any, type = 1) => {
  if (!date) return ''
  switch (type) {
    case 1:
      return moment(date).format("DD/MM/YYYY")
...
export const alertFormat = (data: any, title = 'title') => {
  let res = ''
  if (isObject(data)) {
